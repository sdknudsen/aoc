...
object q {
  ...
  def entryIsSafe(xss: Array[Array[Array[Int]]], i: Int, j: Int): Boolean = {
    var sum = 0
  ...
  def getMinMat(xss: Array[Array[Array[Int]]]): Array[Array[Boolean]] = {
    val arr = Array.ofDim[Boolean](size, size)

    for(i <- 0 until size) {
      for(j <- 0 until size) {
        // arr(i)(j) = getMinEntry(xss, i, j)
        arr(i)(j) = entryIsSafe(xss, i, j)
        }
      }
    arr
  }
  ...
  def main(args: Array[String]) = {
    ...
    val safeMat = getMinMat(mats)
    // showMat(safeMat)
    val count = safeMat.flatten.count(x => x)
    println(count)
      // filter(x => x).length
